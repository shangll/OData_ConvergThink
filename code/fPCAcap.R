...
fPCAcap <- function(sftData, dimensions, acc.cutoff=.75, OR=NULL, stopping.rule=c("OR", "AND", "STST"), ratio=TRUE, register=c("median","mean","none"), plotPCs=FALSE, ...) {
  ...
  if(plotPCs) {
    ...
    plot(c(tmin-1000, tmax+1000), c(0,0), type='l', 
           xlim=c(tmin, tmax), ylim=c(-0.5,1.5),
  ...
  if(plotPCs) {
    ...
    dev.new()
    par(mar=c(3.1, 3.1, 2.1, 1.1), mgp=c(1.75, .25,0), mfrow=c(dimensions,2))
    for ( ifac in 1:dimensions) {
      ...
      plot(tgood, Wveci - capGoodmn, type='l', main="", xlab="Time (Adjusted)", ylab="",
           ylim=c(-0.1, 0.3), xlim=c(tmin, tmax))
      abline(0,0, col=grey(.4))
    ...
    for ( ifac in 1:dimensions) {
      ...
      plot(tgood, Wveci, type='l', lty=2, lwd=2, col="blue", bty="l", main="", xlab="Time (Adjusted)", ylab="",
           ylim=c(-0.5, 1.5), xlim=c(tmin, tmax))
      lines(tgood, capGoodmn, lwd=2)
      abline(0,0, col=grey(.4))
      mtext(mainstr, side=2, line=1)
      ...
      if(ifac==1) {
        ...
        legend("topright", c("Component", "Mean"), bty="n",lty=c(2,1),col=c("blue","black"))
      }

      plot(tgood, Wveci - capGoodmn, type='l', lwd=2, bty="l", main="", xlab="Time (Adjusted)", ylab="",
           ylim=c(-0.1, 0.3), xlim=c(tmin, tmax))
      abline(0,0, col=grey(.4))
      if(ifac==1) {mtext("Component - Mean", side=3, line=.5)}
...
dat <- read.table("ANDsd3Del.txt", header=T)
dat.AND <- subset(dat, dat$Condition == "AND" & dat$Correct == 1)
fPCAcap(dat.AND, dimensions=2, stopping.rule="AND", plotPCs=TRUE)